...
def vflip(brd: Mat) -> Mat:
    return np.flip(brd, axis=1)


def brd2str(brd: Mat) -> str:
    brd = vflip(brd)
    ls = (str(list(l)) for l in (brd[0] * 2 + brd[1])[:])
    return f"Your Move:[{''.join(ls)}]\n"


def str2brd(msg: str, pref: str = 'Your Move:[[', suf: str = ']]') -> Mat:
    assert msg.startswith(pref)
    assert msg.endswith(suf)
    ls = [[c.strip() for c in l.split(',')] for l in msg[len(pref):len(msg) - len(suf)].split('][')]
    mat = np.array(ls, dtype=np.uint8)
    brd0 = mat == 2
    brd1 = mat == 1
    brd = np.concatenate((brd0, brd1), axis=0).astype(dtype=np.bool_)
    return vflip(brd)

def main():
    num = int(input('PayerID: '))
    assert num == 1 or num == 2, f'yo, you can be player 1 or 2 - you tried to be player {num}'
    uid = 1 if num == 1 else 0
    pyg.init()
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as skt:
            skt.connect(('192.168.137.1', 12345))
            skt.sendall(str.encode(f'player{num}'))
            gmst = GameState()
            ...
            while gmst.run:
                    ...
                    if ev.type == pyg.QUIT:
                        ...
                        if ev.key == pyg.K_r:
                            ...
                            if kmods & pyg.KMOD_CTRL:
                                ...
                                # append to gmst.vrts
                                pass
                    elif ev.type == pyg.MOUSEBUTTONDOWN and gmst.ply == uid:
                ...
                if len(gmst.vrts) == 2 and cnd:
                    ...
                    if ok:
                        ...
                        gmst.ply = 1 - gmst.ply
                        skt.sendall(str.encode(brd2str(gmst.brd)))
                        gmst.brd = str2brd(skt.recv(1024).decode())
                if not cnd:
                    gmst.vrts.clear()
    finally:
        pyg.quit()
